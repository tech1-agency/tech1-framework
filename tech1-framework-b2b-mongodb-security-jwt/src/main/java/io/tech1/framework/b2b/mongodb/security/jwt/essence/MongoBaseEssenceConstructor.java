...
public class MongoBaseEssenceConstructor implements EssenceConstructor {
    ...
    public void addDefaultUsers() {
        assertTrueOrThrow(
                this.applicationFrameworkProperties.getSecurityJwtConfigs().getEssenceConfigs().getDefaultUsers().isEnabled(),
        ...
        if (this.mongoUsersRepository.count() == 0L) {
            ...
            var users = defaultUsers.stream().
                    map(defaultUser -> {
                        ...
                        LOGGER.debug(FRAMEWORK_B2B_SECURITY_JWT_PREFIX + " Essence `defaultUsers`. Convert default user. Username: {}", username);
                        return user;
                    })
                    .collect(Collectors.toList());
            this.mongoUsersRepository.saveAll(users);
            LOGGER.warn(FRAMEWORK_B2B_SECURITY_JWT_PREFIX + " Essence `defaultUsers` is completed. Saved dbRecords: `{}`", users.size());
        } else {
    ...
    public void addDefaultUsersInvitationCodes() {
        assertTrueOrThrow(
                this.applicationFrameworkProperties.getSecurityJwtConfigs().getEssenceConfigs().getInvitationCodes().isEnabled(),
