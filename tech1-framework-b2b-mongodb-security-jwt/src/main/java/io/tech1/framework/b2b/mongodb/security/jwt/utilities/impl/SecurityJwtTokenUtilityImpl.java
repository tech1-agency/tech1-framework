...
public class SecurityJwtTokenUtilityImpl implements SecurityJwtTokenUtility {
    ...
    @Override
    public JwtAccessToken createJwtAccessToken(DbUser dbUser) {
        ...
        var jwtToken = this.createJwtToken(dbUser.getUsername(), dbUser.getAuthorities(), accessTokenConfiguration.getExpiration());
        return new JwtAccessToken(jwtToken);
    }

    @Override
    public JwtRefreshToken createJwtRefreshToken(DbUser dbUser) {
        ...
        var jwtToken = this.createJwtToken(dbUser.getUsername(), dbUser.getAuthorities(), refreshTokenConfiguration.getExpiration());
        return new JwtRefreshToken(jwtToken);
    }

    @Override
    public String createJwtToken(Username username, List<SimpleGrantedAuthority> authorities, TimeAmount timeAmount) {
        var claims = Jwts.claims().setSubject(username.getIdentifier());
        claims.put("authorities", authorities);
        return Jwts.builder()
                .setId(UUID.randomUUID().toString())
                .setClaims(claims)
                .setIssuedAt(this.securityJwtDateUtility.getIssuedAt())
                .setExpiration(this.securityJwtDateUtility.getExpiration(timeAmount))
    ...
    public boolean isExpired(JwtTokenValidatedClaims jwtTokenValidatedClaims) {
        return jwtTokenValidatedClaims.isValid() && this.securityJwtDateUtility.getIssuedAt().after(jwtTokenValidatedClaims.getClaims().getExpiration());
    }

    // =================================================================================================================
    // PRIVATE METHODS
    // =================================================================================================================
    public JwtTokenValidatedClaims validate(String jwtToken, boolean isAccess, boolean isRefresh) {
        try {
